                                    ...
                                    <div
                                        ...
                                        <div class="mt-md-0 mt-2">
                                            ...
                                            <div class="invoice-date-wrapper">
                                                ...
                                                <p class="invoice-date">{{ date('d-m-Y', strtotime($order->created_at)) }}
                                                </p>
                                            </div>
                                            <!-- <div class="invoice-date-wrapper">
                                            ...
                                            <table>
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        <td><span
                                                                class="font-weight-bold">${{ $order->final_price - ($order->value_added_tax + $order->incomming_tax + $order->system_commission + $order->shipping_cost) }}</span>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                        ...
                                        <tbody>
                                            @foreach ($orders as $item)
                                                <tr>
                                                    <td class="py-1">
                                                        <a
                                                            href="{{ url(app()->getLocale() . '/products/' . $item->products->id) }}">
...
@else
                                            ...
                                            <div class="invoice-total-wrapper">
                                                <div class="invoice-total-item">
                                                    ...
                                                    <p class="invoice-total-amount">
                                                        {{ $order->order_details->sum('points') }}</p>
                                                </div>
                                                <div class="invoice-total-item">
                                                ...
                                                <div class="invoice-total-item">
                                                    <p class="invoice-total-title">{{ __('common.system_commission') }}:
                                                    </p>
                                                ...
                                                <div class="invoice-total-item">
                                                    ...
                                                    <p class="invoice-total-amount">
                                                        ${{ $order->total - ($order->value_added_tax + $order->incomming_tax + $order->system_commission + $order->shipping_cost) }}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            @can('roles.vendor')
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <button id="status_order"
                                            class="btn  @if ($order->status == 0) btn-outline-info @elseif($order->status == 1) btn-outline-success @elseif($order->status == 2) btn-outline-primary @elseif($order->status == 3) btn-outline-danger @endif btn-block btn-download-invoice mb-75">
                                            @if ($order->status == 0)
                                                {{ __('common.new') }}
                                        ...
                                        @if ($order->status == 0)
                                            <button onclick="change_status({{ $order->id }})" id="btn_accept_order"
                                                class="btn btn-primary btn-block btn-download-invoice mb-75">
                                                {{ __('common.order_accept') }}
                                            </button>
                                        @endif

                                        @if ($order->status != 3)
                                            <div class="btn btn-outline-secondary btn-block mb-75" href="#cancel_order"
                                        ...
                                        <a class="btn btn-outline-warning btn-block mb-75"
                                            onclick="window.print();return false;" target="_blank">
                                            {{ __('common.print_order') }}
                                        </a>

                                        <a class="btn btn-outline-danger btn-block mb-75" target="_blank"
                                            href="{{ url(app()->getLocale() . '/export_pdf_order/' . $order->id) }}">
                                            {{ __('common.export_as_pdf') }} </a>
                                ...
                                <div class="card">
                                    <div class="card-body" style="text-align: center;line-height: 2;">
                                        ...
                                        <div style="text-align:center">
                                            <img src="{{ $order->vendors->data->ID_image }}" height="125px"
    ...
    <!-- END: Content-->
    <!--Form cancel product -->
    <div id="cancel_order" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1"
        aria-hidden="true">
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $('#status_order').text(
                                '{{ __('common.status_order') }} : {{ __('common.accepted') }}')
                        }
                    });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.message === 'Success') {
                            ...
                            $('#status_order').text(
                                '{{ __('common.status_order') }} : {{ __('common.canceled') }}'
                                )
